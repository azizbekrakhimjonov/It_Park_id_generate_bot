...
from buttons import result, start_button
...
async def cm_start(message: types.Message):
    ...
    if message.from_user.id == ID:
        ...
        await message.reply("Введите ID ученика:")
...
async def load_photo(message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        await state.finish()
        await message.reply("Получить ID карту", reply_markup=result)


async def load_make(message: types.Message):
    if message.text == 'Получить ID карту':
        photo_front = InputFile(f"{img_name}.jpg")
        photo_back = InputFile(f"pic3.jpg")
        await bot.send_photo(chat_id=message.chat.id, photo=photo_front)
        await bot.send_photo(chat_id=message.chat.id, photo=photo_back)
